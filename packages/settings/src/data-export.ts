...
function wellnessCheckinsQuery(userId: string) {
  return sql<Record<string, unknown>>`
    SELECT
      ...
      feeling_core AS "feelingCore",
      feeling_secondary AS "feelingSecondary",
      feeling_tertiary AS "feelingTertiary",
...
function medicationsQuery(userId: string) {
  return sql<Record<string, unknown>>`
    SELECT
      ...
      weekdays,
      schedule_times::text[] AS "scheduleTimes",
...
function wellnessTherapyNotesQuery(userId: string) {
  return sql<Record<string, unknown>>`
    SELECT
      ...
      linked_emotion AS "linkedEmotion",
      created_at AS "createdAt",
      updated_at AS "updatedAt"
