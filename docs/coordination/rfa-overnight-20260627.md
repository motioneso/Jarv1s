...
**tab discipline:** coordinator tab (`w1:tS`) = coordinator pane only; all build/QA agents go in agents tab (currently `w1:tW`, label "agents"). Relay successor also spawns in coordinator tab (that's the one exception). If agents tab reaches 4 panes, open "agents 2" in w1. Always Jarvis workspace (w1) — updated 2026-06-27 per Ben directive; skill updated to match.
...
- **#540** (sensitive, w1:p65, session 2e3bf558): BUILDING. R5 active. Tasks 1-11 done (6/7 gate tests committed, purge DELETE policy fix in migration d5bae171). R5: apply policy to dev DB, recreate jarvis_build_540, full gate, wrap-up. Migration slot 0128 (XXXX placeholder). R4 (w1:p63, cc947624) REAPED.
- **#541** (routine, w1:p66, session 594e1975): BUILDING. R4 active. Plan APPROVED (11 TDD tasks). R3 mislabeled commits — actual remaining: Tasks 7-11. R4 building from task 7 now. Collision: shares chat-api.ts with #539 (disjoint); second to merge needs rebase. R3 (w1:p5X, abf19e12) REAPED.
...
**#531 MERGED ✅** — PR #563 squash-merged 2026-06-28T04:03:45Z. Dual-model QA: Opus R4 GREEN + AGY R4 GREEN. Issue #531 closed. Follow-up #567 filed (monitor-provider unsanitized title/location). ⚠️ failure-budget hit 2 during #531 QA.
...
| #536 | `docs/superpowers/specs/2026-06-27-scheduled-recurring-jarvis-briefings.md` | sensitive | **MERGED** PR #568 `d9b798c5`; issue closed; board Done; worktree removed | Claude/Sonnet | R2 QA GREEN (ace1a320) | `rfa-536-scheduled-briefings` (merged) | #568 (merged) |
| #537 | `docs/superpowers/specs/2026-06-27-automatic-commitment-extraction.md` | security | **MERGED** ✅ PR #570 @ 6835a9d0 2026-06-28T09:04:23Z. Dual QA: Opus GREEN (10/10 + 1 correctness fix dec6f074) + QA2 GREEN. Issue #537 closed. Board Done. Worktree removed. | Claude/Sonnet | Opus GREEN + QA2 GREEN | `rfa-537-commitment-extraction` (merged) | #570 (merged) |
| #538 | `docs/superpowers/specs/2026-06-27-unified-person-contact-model.md` | security | **BUILDING** — R3 (Sonnet, w1:p69, session e8e2e519). R2 (877d7818) died on API stall mid-plan-write (37min cook, pane now reaped). R3 checking partial plan + completing. Plan NOT yet approved — coordinator must approve before coding (security tier). Migration slot: 0127 (XXXX placeholder). | Claude/Sonnet | Opus+GLM dual security QA (TBD) | `rfa-538-person-contact-model` | - |
| #557 | `docs/superpowers/specs/2026-06-28-calendar-delete-tool.md` | security | **MERGED** ✅ PR #569 @ 020f36af. Dual QA: Opus GREEN (10/10) + QA2 GREEN (5 adversarial). Issue #557 closed. Board → Done. Worktree removed. | Claude/Sonnet | Opus GREEN + QA2 GREEN | `rfa-557-calendar-delete` (merged) | #569 (merged) |
| #540 | `docs/superpowers/specs/2026-06-28-safe-automation-audit-log.md` | sensitive | **BUILDING** — R5 (Sonnet, w1:p65, session 2e3bf558). R4 (cc947624) reaped. Tasks 1-11 done (6/7 gate tests committed, purge DELETE policy fix commit d5bae171). R5: apply policy to dev DB, recreate jarvis_build_540, run full gate, wrap-up. Migration slot: 0128 (XXXX placeholder). | Claude/Sonnet | Codex+GLM dual QA (TBD) | `rfa-540-safe-automation-audit-log` | - |
| #539 | `docs/superpowers/specs/2026-06-28-source-backed-answers-provenance.md` | sensitive | **QA IN PROGRESS** — PR #571 open. VF_EXIT=0 AUDIT_EXIT=0 (1190 pass/2 skip/92 files). CI "Verify foundation and app" still pending. Codex R1 reaped (verdict: invariants:pass, CI:pending — counted as pass once CI green). GLM crashed → GLM R2 spawned (w1:p6B, ses_0f23, working). Awaiting GLM R2 verdict + CI green. | Claude/Sonnet | Codex invariants:pass + GLM R2 (w1:p6B) working | `rfa-539-source-backed-provenance` | #571 |
| #541 | `docs/superpowers/specs/2026-06-28-data-freshness-visibility.md` | routine | **BUILDING** — R4 (Sonnet, w1:p66, session 594e1975). R3 (abf19e12) reaped. Plan APPROVED (11 TDD tasks, no migration). R3 mislabeled commits — actual remaining: Tasks 7-11 (resolveChatFreshness, readSourceFreshness missing from source; R4 building from task 7). Collision note: shares chat-api.ts with #539 (disjoint fields; second to merge needs rebase). | Claude/Sonnet | Codex+GLM dual QA (TBD) | `rfa-541-data-freshness-visibility` | - |
