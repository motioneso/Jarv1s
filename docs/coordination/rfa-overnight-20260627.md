...
**Continuation note (latest — successor reads this):** Coordinator `75373f84` driving. #525 MERGED via PR #558 at `65ff462c` (2026-06-27T23:29:54Z), sensitive tier, QA GREEN (0 blocking, 2 non-blocking); issue #525 closed; board Done; worktree/pane reaped. #531 (session `c5b4a312`, label `RFA-531 Claude`): T15 done at `5d5581c1`; T16 cleared — no module-registry conflict with #525; proceed with T17+T18 after rebase. Migration slot 0122. #533 PR #559 open — QA RED (Opus): 1 blocker = acceptCandidate not overriding confidence/provenance on promoted fact (defaults 0.6/inferred, spec §6 violation); relayed fix to rfa-533-b; CI pending. Re-QA after fix + CI green. Next migration slot: 0123. Queued: #535 (after #533), #536 (after #531+#535). **Post-run task (final coordinator):** File a GitHub issue for "Jarvis what's new / capabilities doc" — tool-call approach (NOT system prompt injection, keep context lean); a single tool Jarvis can call when asked "what can you do?" or "what's new?"; file is static, committed to repo, baked into the image at build time (no deploy trigger — deploy = docker compose up with updated image, no install.sh). Ben's preference: tool over injection, judgement call on format. Ben asked 2026-06-27.
