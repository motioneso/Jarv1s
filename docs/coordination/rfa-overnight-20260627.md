...
| #538 | `docs/superpowers/specs/2026-06-27-unified-person-contact-model.md` | security | **BUILDING** — R5 (w1:p6F) active. Plan APPROVED 2026-06-28 (13 TDD tasks, written by background Agent a20cd535 in 5 commits). Migration slot: 0127 (XXXX placeholder until push). Security checklist embedded in plan. | Claude/Sonnet | Opus+GLM dual security QA (TBD) | `rfa-538-person-contact-model` | - |
...
| #541 | `docs/superpowers/specs/2026-06-28-data-freshness-visibility.md` | routine | **QA RED — FIX IN PROGRESS** — PR #572. Dual QA: GLM RED (CI fail: chat-session-manager-provenance.test.ts:80 recordTurn 5th arg not updated); Codex RED (CI fail + functional: SSE/fallback records omit sourceFreshness — footer only shows after reload). R4-541 (w1:p66) fixing both. Re-QA after push. | Claude/Sonnet | Codex RED + GLM RED — fix in progress | `rfa-541-data-freshness-visibility` | #572 |
