...
| #531 | `docs/superpowers/specs/2026-06-27-restrained-proactive-monitoring.md` | security | PR #563 CI IN_PROGRESS on head `168aeb95` (FIX-531-3: Map-by-title fix + 143-line unit test; all local gates green); round-4 dual QA pending CI green; ⚠️ failure-budget=2 note for relay | Claude Sonnet | round-4 Codex+AGY QA pending after CI green on 168aeb95 | `rfa-531-proactive-monitoring` | #563 |
| #535 | `docs/superpowers/specs/2026-06-27-long-running-jarvis-goals.md` | security | PR #566 head `2af4ede5` — CI IN_PROGRESS (smokes PASS; Verify foundation pending run `28310316916`); fix: stale pnpm-lock.yaml now staged+pushed | AGY | dual-model QA pending CI green | `rfa-535-long-running-goals` | #566 |
