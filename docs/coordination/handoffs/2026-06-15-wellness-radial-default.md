...
**Worktree:** ~/Jarv1s/.claude/worktrees/wellness-radial-default **Branch:** wellness-radial-default (off origin/main @ 2a72ce1)
**Build skill path (absolute):** ~/Jarv1s/.claude/worktrees/wellness-radial-default/.claude/skills/coordinated-build/SKILL.md
...
- Work **only** in this worktree/branch. Commit green per task; `git add` only that task's files (`Co-Authored-By: Claude`). NEVER `git add -A`/`git add .` — other sessions share the tree.
