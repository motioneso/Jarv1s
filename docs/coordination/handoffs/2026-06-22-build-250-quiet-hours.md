...
**Worktree:** ~/Jarv1s/.claude/worktrees/feat-250-quiet-hours **Branch:** feat-250-quiet-hours (off origin/main `201c692`)
**Build skill path (absolute):** ~/Jarv1s/.claude/skills/coordinated-build/SKILL.md (use this exact path if `coordinated-build` does not resolve by name in your spawn env)
...
- Work **only** in this worktree/branch. Commit green per task; `git add` only that task's files (`Co-Authored-By: Claude` — match your real model).
