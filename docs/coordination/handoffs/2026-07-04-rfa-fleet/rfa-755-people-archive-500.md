# Build Handoff — rfa-755-people-archive-500

**Spec (approved):** GitHub issue #755 itself. Ben (2026-07-04) confirmed this audit-finding bug
report is detailed enough to build from directly — no separate `docs/superpowers/specs/` doc for
this one; the issue body IS the approved spec. Read `gh issue view 755` IN FULL before planning.
**GitHub issue:** #755 — `bug(people): PATCH/archive person 500s when notes folder configured but
person has no canonical note`
**Risk tier:** `routine` (board-triaged P1, but pure bug fix — no schema/auth/secret surface).
**Worktree:** `~/Jarv1s/.claude/worktrees/rfa-755-people-archive-500` **Branch:**
`rfa-755-people-archive-500` (off `origin/main@32f34d4b`, post-#737 merge)
...
2. Read GitHub issue #755 IN FULL (`gh issue view 755`) — this is your spec.
...
- Scope is People/Notes archive-path only (the 500 repro is PATCH/archive person when a notes
  folder is configured but the person has no canonical note yet). Stay inside the People module's
  own files.
- No overlap expected with #756 (also People module, but the notes-suggest-updates toggle path) or
  #758 (broad hygiene batch, not touching People per the coordinator's collision check) — both are
  running in parallel in sibling worktrees. If you find yourself touching a file also claimed by
  #756's toggle work, stop and escalate to the coordinator before proceeding.
