...
- **#59** "workspace" vocabulary — genuine fork: _remove the vocabulary_ vs. _spec real workspace
  scoping_ (ADR). Decide the direction before any agent touches manifests.
...
> Aggressive alternative: launch all 10 at once and gate only the _merges_ in the order above.
