...
| **D — Settings → DataContextDb**         | #95 SettingsRepository raw Kysely, #155 /api/me cross-user read                                                                                         | security                           | maybe (grant)                                                  | `docs/superpowers/specs/2026-06-12-audit-slice-d-settings-datacontext.md` ✅    | **BUILDING** — SliceD-build pane `w653f42bef3ac02-5`, branch `audit-slice-d`, `jarvis_qa_d`. Plan Fable-approved (3 rounds, all findings fixed). Handoff at `docs/coordination/handoffs/2026-06-12-audit-slice-d.md`. Unblocks E + H on merge.                                                                                                                                                                                                                                   |
| **E — Auth module hardening**            | #101 module-isolation, #127 bootstrap actor-GUC, #141 OAuth error-body leak (#113 deferred → issue #183)                                                | security                           | no (code)                                                      | `docs/superpowers/specs/2026-06-12-audit-slice-e-auth-hardening.md` ✅          | **HELD (blocked on D)** — SliceE-build pane `w653f42bef3ac02-4` HELD. E plan pre-flight requires `insertAuditEvent(db: DataContextDb)` on origin/main (D's conversion). Build agent confirmed blocked, coordinator confirmed HOLD. Unhold immediately after D merges to origin/main: message E agent "D merged — rebase on origin/main and proceed."                                                                                                                             |
