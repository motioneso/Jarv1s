...
| **F — AI tool-path hardening**           | #132 REST validateToolInput, #119 server-side allowlist, #148 blank ToolContext, #172 tools/list actor-scope                                            | security                           | no (code)                                | `docs/superpowers/specs/2026-06-12-audit-slice-f-ai-toolpath-hardening.md` ✅   | **Fable APPROVE (verdict on PR), but CONFLICTING** — PR #184 swept in 5 stray docs/coordination edits → conflict w/ main prettier fix; SliceF-build stripping them (code unchanged, approval holds); merge on green CI |
| **G — Data-layer defense-in-depth**      | #102 assertDataContextDb, #144 vectorSearch owner predicate, #99 structured-state WITH CHECK                                                            | security                           | maybe (#99, assigned at merge after B+D) | `docs/superpowers/specs/2026-06-12-audit-slice-g-datalayer-defense.md` ✅       | **done, needs scope-strip + QA** — PR #186; swept in 5 stray docs/coordination edits → SliceG-build stripping them; spine-blocked behind B+D for its #99 migration number (must NOT hardcode); QA after clean          |
...
| **I — Portability + observability tail** | #170 export omits private, #149 handleRouteError, #140 list ownership, #166 test hygiene (LOW)                                                          | sensitive/routine→**security-bar** | no (code)                                | `docs/superpowers/specs/2026-06-12-audit-slice-i-portability-tail.md` ✅        | **CI red was COORDINATOR's broken docs, NOT I's code** — failing log listed only docs/coordination/\*.md; main fixed @ 87189c5; SliceI-build told to rebase + re-run → expect green; then QA                           |
