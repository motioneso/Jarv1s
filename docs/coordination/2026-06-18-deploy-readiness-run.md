...
**merges_since_relay:** 1 (sensitive merge #336/#151 at 2026-06-19T21:36Z; relay at 2 routine/sensitive merges)
**last_alive:** 2026-06-19T22:07Z (Codex `019ee1c8…`; #335 QA RED sent to builder)
...
- **Build-238-Export** (GLM/opencode, pane `w1:p49`, Agents tab `w1:tD`) — #238 data export, **sensitive**, branch+worktree `data-export-238`, DB `jarvis_build_238`, status `/tmp/build-238-status.txt`. Fixing QA RED: real route registration, real UI/client export call, and audit baseline check. On new BUILD_DONE: verify committed/pushed head, diff-scan for gate/config tampering, independent gate on merge result, GLM-or-Codex review + sensitive invariant check, then merge if GREEN. Collides with #239 on `packages/settings`.
