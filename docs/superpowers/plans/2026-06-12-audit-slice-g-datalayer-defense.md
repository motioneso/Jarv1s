...
- [ ] **Run the test — expect FAIL:**
  ...
  ```bash
  cd /home/ben/Jarv1s && set -o pipefail && pnpm test:memory 2>&1 | grep -A3 "assertDataContextDb guard"
  ```
...
- [ ] **Run the test — expect PASS (assert exit 0):**
  ...
  ```bash
  cd /home/ben/Jarv1s && pnpm test:memory; echo "exit=$?"
  ```
...
- [ ] **Verify the current SQL is missing the owner predicate (expected: match):**
  ...
  ```bash
  grep -A 15 "async vectorSearch" /home/ben/Jarv1s/packages/memory/src/repository.ts | grep "owner_user_id"
  ```
...
- [ ] **Run acceptance grep — expect match:**
  ...
  ```bash
  grep -A 20 "async vectorSearch" /home/ben/Jarv1s/packages/memory/src/repository.ts | grep "owner_user_id = app.current_actor_user_id()"
  ```
...
- [ ] **Verify callers are unaffected (no signature change):**
  ...
  ```bash
  grep -n "vectorSearch" /home/ben/Jarv1s/packages/memory/src/retrieval.ts /home/ben/Jarv1s/packages/chat/src/recall-port.ts
  ```
...
- [ ] **Run the test — expect FAIL:**
  ...
  ```bash
  cd /home/ben/Jarv1s && set -o pipefail && pnpm test:structured-state 2>&1 | grep -A3 "assertDataContextDb guards"
  ```
...
- [ ] **Run the test — expect PASS (assert exit 0):**
  ...
  ```bash
  cd /home/ben/Jarv1s && pnpm test:structured-state; echo "exit=$?"
  ```
...
- [ ] **Run typecheck — expect clean (prove the intermediate Task 3 commit is gate-green):**
  ...
  ```bash
  cd /home/ben/Jarv1s && pnpm typecheck; echo "exit=$?"
  ```
...
- [ ] **Verify current compile state — expect TypeScript errors after the interface change.** Before editing, confirm the test file call count:
  ...
  ```bash
  grep -c "ownerUserId" /home/ben/Jarv1s/tests/integration/structured-state.test.ts
  ```
...
- [ ] **Confirm no production callers remain with `ownerUserId` in structured-state context:**
  ...
  ```bash
  grep -rn "ownerUserId" /home/ben/Jarv1s/packages/structured-state/src/ --include="*.ts"
  ```
  ...
  grep -rn "CommitmentsRepository\|EntitiesRepository\|PreferencesRepository" \
    /home/ben/Jarv1s/apps /home/ben/Jarv1s/packages --include="*.ts" \
...
- [ ] **Grep: every public method in `memory/repository.ts` has `assertDataContextDb(scopedDb)` at entry:**
  ...
  ```bash
  grep -n "assertDataContextDb(scopedDb)" /home/ben/Jarv1s/packages/memory/src/repository.ts
  ```
...
- [ ] **Placement check: the guard is the FIRST statement of each method body, before any `await`.**
  ...
  grep -B1 "assertDataContextDb(scopedDb)" /home/ben/Jarv1s/packages/memory/src/repository.ts \
    | grep -c "await"
  ```
...
- [ ] **Grep: every public method in `structured-state/src/*.ts` has `assertDataContextDb(scopedDb)` at entry:**
  ...
  ```bash
  grep -rn "assertDataContextDb(scopedDb)" /home/ben/Jarv1s/packages/structured-state/src/
  ```
...
- [ ] **Placement check (structured-state): the guard is the FIRST statement, before any `await`:**
  ...
  ```bash
  grep -rB1 "assertDataContextDb(scopedDb)" /home/ben/Jarv1s/packages/structured-state/src/ \
    | grep -c "await"
  ```
...
- [ ] **Confirm no migration was added (migration count stays 0 for this slice).** Use git, not
  ...
  ```bash
  cd /home/ben/Jarv1s && git status --porcelain \
    packages/memory/sql packages/structured-state/sql infra/postgres/migrations
  cd /home/ben/Jarv1s && git diff --stat main -- \
    packages/memory/sql packages/structured-state/sql infra/postgres/migrations
  ```
...
- [ ] **Run full foundation gate (assert exit 0):**
  ...
  ```bash
  cd /home/ben/Jarv1s && pnpm verify:foundation; echo "exit=$?"
  ```
