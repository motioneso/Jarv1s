...
- No committed files under `~/Jarv1s/JarvisProd`.
...
- Type and path consistency: provider names use the existing `openai-compatible` literal; prod-only commands stay under `~/Jarv1s/JarvisProd`; committed files are limited to `Dockerfile` and this plan.
