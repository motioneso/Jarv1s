...
```bash
cd ~/Jarv1s && pnpm --filter @jarv1s/web typecheck
```
...
```bash
cd ~/Jarv1s && pnpm --filter @jarv1s/web typecheck && pnpm --filter @jarv1s/web lint
```
...
```bash
cd ~/Jarv1s && pnpm exec playwright test chat-drawer --reporter=line; echo "EXIT=$?"
```
...
```bash
cd ~/Jarv1s && pnpm check:file-size && pnpm build:web; echo "EXIT=$?"
```
