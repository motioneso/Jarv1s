...
The spec's grep was `"Jarvis` — a double quote _immediately_ before the word. It only matches
...
**Tie-breaker:** substitute _"the app"_ into the sentence.

- Reads correctly → **product** → literal `Moss`.
- Needs _"your assistant"_ → **assistant** → runtime read of `useAssistantName()`.
...
| ------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `composed prompt names the configured assistant exactly once` | With `persona.assistantName = "Alfred"`, the composed prompt's occurrences of `Alfred` === 1                                                                                 | Today `:60` hardcodes `Jarvis` while `persona-api.ts:67` emits `Your name is Alfred.` — two identities. A naive fix that _substitutes_ the name into `:60` instead of deleting it yields 2 and fails. |
| `composed prompt contains no hardcoded assistant name`        | Composed prompt does not match `/Jarvis/i`, and does not contain `Moss` in an _identity_ position (`You are Moss`, `Your name is Moss`) when the configured name is `Alfred` | Catches a rename-not-remove fix that turns `:60` into `"You are Moss, …"` — which reintroduces the exact bug under the new name.                                                                      |
...
**Known environment ceiling, disclosed not worked around:** chat _turns_ 503 on host-dev (no
...
| R5  | Spec's frozen-directory file counts are stale (plans 264→296, specs 214→248, coordination 115→61, audits 56→24). The _rule_ is unaffected; counts are not a checksum.                                                                      | `ls` on this branch                                                                            | 2026-08-06            |
| R6  | The census was scoped to `apps/web/src` and **missed six display strings in `packages/`**. Scoping the residue grep to the app directory is not sufficient — shipped display strings also live in the shared app map and module manifests. | `app-map-core.ts:87,94,115`, `manifest.ts:65,204`, `live-routes.ts:537`                        | 2026-08-06            |
| R7  | `app-map-core.ts` assistant descriptions take the generic noun (`the assistant`), not a threaded name. They are static module-scope data consumed by the model via `app.getMapSlice`; the file already used this idiom.                    | `app-map-core.ts:100` — `Review assistant activity visible to this user.` predates #1441       | 2026-08-06            |
| R8  | The evening-interview prompt's identity is **deleted, not threaded** — same ruling as R3, but here it is also mandatory. Interpolating a name into `<trusted_instructions>` would break the prompt-isolation invariant.                    | `briefings-prompt-isolation.test.ts:86` asserts that block is a pure literal, no interpolation | 2026-08-06            |
| R9  | Converting a module-scope constant to a function of `assistantName` breaks consumers in `tests/`, which sat outside every lane's file set. Four such breaks occurred; a signature change must sweep `tests/` in the same pass.             | `settings-source-behaviors.test.ts:4` TS2724 redded the first gate at `pnpm typecheck`         | 2026-08-06            |
