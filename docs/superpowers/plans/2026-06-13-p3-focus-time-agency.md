...
```bash
git -C /home/ben/Jarv1s fetch origin && git -C /home/ben/Jarv1s log origin/main --oneline | head -5
grep -rl "class GoogleApiClient" /home/ben/Jarv1s/packages/connectors/src/
ls /home/ben/Jarv1s/packages/calendar/sql/0065_calendar_worker_grants_and_google_insert.sql
grep -n "upsertCachedEvent" /home/ben/Jarv1s/packages/calendar/src/repository.ts
```
...
Run: `grep -nE '"test:integration"|"verify:foundation"|tests/integration' /home/ben/Jarv1s/package.json`
...
```bash
grep -rhoE 'var\(--[a-z0-9-]+' /home/ben/Jarv1s/apps/web/src --include='*.css' --include='*.tsx' | sort -u
```
...
```bash
grep -rnE "getByRole|getByTestId|locator\(|getByLabel|data-testid|aria-label" /home/ben/Jarv1s/tests/e2e/*.spec.ts
```
...
```bash
wc -l /home/ben/Jarv1s/apps/web/src/styles.css
```
...
```bash
grep -rlE '#[0-9a-fA-F]{3,6}|rgb\(' /home/ben/Jarv1s/apps/web/src --include='*.css'
```
...
```bash
grep -rnE "@jarv1s/shared|api-client|useQuery|fetch\(" /home/ben/Jarv1s/apps/web/src/ui/ 2>/dev/null
```
...
Then: `grep -rnE "@jarv1s/shared|api-client|useQuery|fetch\(" /home/ben/Jarv1s/apps/web/src/ui/` → expect **no
matches**.
...
```bash
ls -la /home/ben/Jarv1s/docs/brand/mockups/
grep -lE '#[0-9a-fA-F]{3,6}' /home/ben/Jarv1s/docs/brand/mockups/*.html  # hex is fine in mockups (self-contained)
```
...
```bash
grep -nE '#[0-9a-fA-F]{3,6}|rgb\(' /home/ben/Jarv1s/apps/web/src/styles.css
```
...
grep -rlE '#[0-9a-fA-F]{3,6}|rgb\(' /home/ben/Jarv1s/apps/web/src --include='*.css'   # only tokens.css
```
...
grep -rn "@jarv1s/connectors" /home/ben/Jarv1s/packages/calendar/src/ && echo "VIOLATION" || echo "OK: calendar has no connectors import"
# no token/secret in the action-request summary path (summarizeAssistantToolInput is key-names-only — unchanged)
grep -rn "accessToken\|refreshToken\|client_secret\|encryptedSecret" /home/ben/Jarv1s/packages/calendar/src/ && echo "CHECK" || echo "OK: no secret names in calendar"
