...
- **`ProviderInstallState`** (base §9.2 enum, `onboarding-api.ts:44-50`) and the persistence table
  ...
  **verbatim** — this addendum does not add states or columns. It only defines **who writes them when**.
  - **Table-name reconciliation (authoritative):** the base contract §9.2 _sketched_ the table as
...
export const PROVIDER_CATALOG: ProviderCatalog = {
  ...
  "openai-compatible": {
    ...
    recipe: {
      ...
      selfUpdateDisable: {
        kind: "config",
        path: "<PINNED_CODEX_CONFIG_PATH>",
        content: "<PINNED_CODEX_DISABLE_UPDATE>"
      }
    }
  },
  google: {
...
| download/verify/promote failure                                                                                       | returned as `RpcInstallProviderResult{ state:"error", message }` on an **`RpcOk`** | a _failed install_ is a normal terminal OUTCOME, not a transport error — the verb succeeds, the result says `error`. Reserve `RpcErr internal` for an unexpected server fault (e.g. the lock impl throws).                                                             |
...
exposure introduced by this addendum): the controls here ensure the _installed_ code is pinned, checksummed,
