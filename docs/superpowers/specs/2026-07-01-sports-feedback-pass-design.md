...
**Status:** approved — Ben resolved all §10 gate questions 2026-07-01 (answers folded into D-1, D-7, D-8).
**Date:** 2026-07-01
**Owner:** Ben + hive
...
- **Top Stories (rail, sticky, replaces "Headlines"):** deterministic ranking — (1) headlines tagged with a followed team, newest first; (2) then the newest headline of each followed competition not already included; cap 6 (Ben 2026-07-01). The story hero uses `topStories[0]` (so the hero is also relevance-ranked, not "first headline of first competition").
- **League news (new full-width section below Scores):** one group per followed competition, all remaining headlines excluding anything already in Top Stories — **no hard cap** (Ben 2026-07-01). The list is naturally bounded: one news-endpoint page per competition (ESPN returns roughly 10–25 articles), minus the Top Stories dedup. Grid card = optional thumbnail (`imageUrl`) + mono competition label + serif linked title — composed from the existing `sp-fcgrid`/`sp-hl` idioms, no new tokens, no new colors, no left-border accents.
...
| #   | Decision                                                                                                                                                             | Rationale                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| --- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| D-1 | **CSP host allowlist** from `SportsSource.imageHosts`, **not** an image proxy — **approved (Ben 2026-07-01)**                                                        | A URL-fetching proxy re-creates the SSRF surface the v0.1.0 audit flagged as the top finding and adds bandwidth/cache machinery. A static CSP host list has zero server attack surface. Cost: the browser hotlinks ESPN's CDN (client IP visible to ESPN for public sports images) — same exposure class the MVP design already accepted by shipping `crestUrl`. **TOS note (Ben asked):** hotlinking `a.espncdn.com` images sits in the same unlicensed gray zone as the unofficial ESPN JSON this module already consumes (spec #656 D2 accepted that for self-hosted personal use) — it adds no new class of exposure, there is no key or account to revoke, and if ESPN ever adds referer blocking the images fail closed to the initials swatch. Proxy remains a documented fast-follow if the privacy posture tightens. |
...
| D-7 | Compose existing `sp-*` idioms for the new sections; no new Claude Design mock gate — **confirmed (Ben 2026-07-01: "no mock data / gate for any of these in prod")** | This is a functionality pass per Ben's established split; the mock's design language (§4.6a of spec #656) already defines every primitive the new sections use. Ben annotates visuals in a later pass. Corollary of Ben's answer: prod never renders fabricated/mock content — authored placeholders and initials swatches appear only as degraded/empty states when real data is absent.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| D-8 | Caps: **6** Top Stories, **no hard cap** on league-news headlines per competition — **revised per Ben 2026-07-01**                                                   | Ben: "Capping the headlines is good, I could see it as 6 instead of 5. We don't need a HARD cap for league-news items below scores." Top Stories stays a deterministic, testable constant (6) in the service; league news renders everything the per-competition news fetch returned (one ESPN news page, ~10–25 articles) minus Top Stories duplicates.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- Top Stories caps at 6 with followed-team stories ranked first; league news grid excludes Top Stories duplicates and renders all remaining headlines from the per-competition fetch (no hard cap).
...
## 10. Approval-gate questions — resolved (Ben, 2026-07-01)

1. **D-1 image delivery:** ~~allowlist vs proxy?~~ → **Allowlist approved** ("Yes that's fine"). Ben's follow-up on hotlinking TOS is answered in the D-1 rationale: same unlicensed gray zone as the ESPN JSON already shipped, no new exposure class, graceful failure if ESPN ever blocks hotlinks.
2. **D-7 design-mock gate:** ~~confirm no mock gate?~~ → **Confirmed** ("There should be no mock data / gate for any of these in prod") — no mock gate, and no mock/fabricated data rendered in prod (see D-7 corollary).
3. **D-8 caps:** ~~5 top stories / 6 per league?~~ → **Revised**: 6 Top Stories; no hard cap on league news below Scores.
