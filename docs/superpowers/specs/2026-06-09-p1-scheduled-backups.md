...
3. **Vault not included** — the Obsidian vault at `/home/ben/obsidian-vault` contains the raw
...
`User=ben` in the service unit, or a dedicated service user). **Two files are added to the repo
...
jarv1s-<timestamp>/
  ...
  vault/           ← rsync-style copy of /home/ben/obsidian-vault
```
...
Off-host copy goes via **rsync over SSH to the Tailscale host `100.76.73.69`** (Ben's Windows box,
...
the vault at `/home/ben/obsidian-vault` and `backups/` under `/home/ben/Jarv1s/` are both
...
```
infra/systemd/
  jarv1s-backup.service    ← runs the wrapper script as User=ben
...
EnvironmentFile=/home/ben/Jarv1s/infra/backup.env   # ← holds PGPASSWORD + opt offhost cmd
