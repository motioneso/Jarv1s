...
- Postgres up (`jarv1s-postgres`, db `jarv1s`). The dev DB was reset by integration-test runs —
  the pilot/test users are gone; `bendlove@gmail.com` is the only real user (from manual testing).
- Logs: `/home/ben/webwright/jarvis-chat-phase1/logs/{api,web,worker}.log`. Manual UI-test
