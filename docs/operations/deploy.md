...
| **≥1 provider CLI** — `claude` / `codex` / `gemini` (+ its `login`) | Live AI chat drives your installed, logged-in CLI. Install at least one and run e.g. `claude login`. `install.sh` warns if none are on `PATH`. The container cannot see host binaries (only their auth dirs are mounted, per ADR 0008), so `install.sh` records the CLIs it detects into `JARVIS_HOST_CLIS` (comma-separated, e.g. `claude,codex,gemini`) in `env.production.local`; the API consults that list — not a container PATH probe — for onboarding provider detection. Empty/unset falls back to a local PATH probe (the non-containerized path). |
...
4. **Records bridge paths** — appends the host CLI dirs, chat home, and tmux
   socket dir to `env.production.local` so the file is fully self-contained. It
   also appends `JARVIS_HOST_CLIS` (the provider CLIs detected on PATH) so the
   containerized API — which cannot see host binaries — can still report them as
