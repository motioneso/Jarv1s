...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        create_btn = form.getByRole("button", name="Create account")
        expect(create_btn).toBeVisible()
        ...
        page.getByLabel("Name", exact=True).fill(SANITIZED_NAME)
        page.getByLabel("Email", exact=True).fill(SANITIZED_EMAIL)
        page.getByLabel("Password", exact=True).fill(SANITIZED_PASSWORD)
        create_btn.click()
        page.wait_for_load_state("networkidle")
        log(f"step 2 action: submitted sanitized owner signup ({SANITIZED_NAME} / {SANITIZED_EMAIL})")

        skip_setup = page.getByRole("button", name="Skip setup")
        expect(skip_setup).toBeVisible(timeout=15000)
        screenshot(page, "final_execution_2_onboarding_wizard.png")
        skip_setup.click()
        log("step 3 action: onboarding wizard appeared; clicked 'Skip setup'")

        skip_anyway = page.getByRole("button", name="Skip anyway")
        expect(skip_anyway).toBeVisible(timeout=10000)
        ...
        expect(usermenu).toBeVisible(timeout=15000)
