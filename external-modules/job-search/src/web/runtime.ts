...
// Task 18 (#1302): typed accessors over the frozen host runtime global
// (finance/src/web/runtime.ts is a verbatim port of this file's shape — see its own header).
// The bundle must never carry its own React (bundle-hygiene precedent): every hook/JSX call
// delegates to the host's instance. Captured at module scope because the host installs the
// global at app boot before any bundle import; a missing global means a broken host, and the
...
function readRuntime(): ModuleRuntime {
  ...
  if (!runtime || runtime.contractVersion !== 2) {
    throw new Error("job-search web root requires the Jarvis module runtime v2");
  }
  return runtime;
}
