// external-modules/finance/src/web/runtime.ts
// FIN-02 (#1147): typed accessors over the frozen host runtime global, ported
// from job-search's web contract. The bundle must never carry its own React
// (bundle-hygiene test) — everything delegates to the host instance. Captured
// at module scope: the host installs the global at app boot before any bundle
// import, so a missing global means a broken host and the loader's dynamic
// import fails closed to Missing.
...
function readRuntime(): ModuleRuntime {
  ...
  if (!runtime || runtime.contractVersion !== 2) {
    throw new Error("finance web root requires the Moss module runtime v2");
  }
  return runtime;
}
