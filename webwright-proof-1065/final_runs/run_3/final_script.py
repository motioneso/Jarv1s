"""PR #1065 live UAT — host-truth (issue #993), run_3.

Re-grounded on exact PR #1065 head SHA 9976ab6b. See ../../plan.md for critical
points. Fresh Firefox headless session, real signup + real Settings navigation,
real one-click Herdr install through the UI. No installer stdout/stderr is ever
parsed or surfaced; only the structured {state, herdrInstalled} result and the
structured diagnostics JSON are read, plus a secret-marker scan over every
response body and DOM snapshot.
"""
import json
from playwright.sync_api import sync_playwright

BASE = "http://localhost:5176"
EMAIL = "uat1065.run3.owner@example.com"
...
with sync_playwright() as p:
    ...
    log("step 0 grounding: PR #1065 head SHA 9976ab6b; api PATH scrubbed of real herdr; prefix bin reset to not-installed before run")

    log("step 1 action: navigate to fresh app, create owner account via real signup UI")
    page.goto(BASE, wait_until="load")
    page.get_by_label("Name").fill("UAT Owner Run3")
